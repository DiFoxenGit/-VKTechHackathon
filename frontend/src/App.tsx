...
import { draftDeck, presentationService, slideThumbnail, templateThumbnail, toOutline } from './services/presentationService';
import type { ThumbWidth } from './services/presentationService';
...
/** Какие колоды сервера созданы в этом браузере: без учётных записей это
 *  единственный способ не показывать посетителю чужие презентации. */
const OWN_KEY = 'slide-studio:own';

function readOwn(): Set<string> {
  try {
    const ids = JSON.parse(localStorage.getItem(OWN_KEY) || '[]');
    return new Set(Array.isArray(ids) ? ids.filter((id): id is string => typeof id === 'string') : []);
  } catch { return new Set(); }
}

function rememberOwn(ids: string[]) {
  const own = readOwn();
  ids.forEach(id => own.add(id));
  try { localStorage.setItem(OWN_KEY, JSON.stringify([...own].slice(-200))); }
  catch { /* Без хранилища список просто останется пустым. */ }
}
...
function issueWord(value: number): string {
  return plural(value, 'замечание', 'замечания', 'замечаний');
}
...
function TemplateArt({ template, compact = false }: { template: Template; compact?: boolean }) {
  return <ServerImage path={templateThumbnail(template.id)} alt={`Обложка шаблона «${template.name}»`} className={`template-art template-thumb ${compact ? 'compact' : ''}`} fallback={<TemplateMock template={template} compact={compact} />} />;
}

/** CSS-набросок шаблона: только пока сервер не прислал настоящую обложку. */
function TemplateMock({ template, compact = false }: { template: Template; compact?: boolean }) {
...
type SlideSource = { id: string; revision?: number | string } | null | undefined;

/** Слайд так, как он выглядит в выгруженном файле. Пока картинка едет с сервера
 *  (или колода ещё не сохранена) — CSS-набросок с цветом и шрифтом шаблона. */
function SlideView({ source, width, ...canvas }: { source: SlideSource; width: ThumbWidth } & Parameters<typeof SlideCanvas>[0]) {
  const index = canvas.index ?? 0;
  const path = source ? slideThumbnail(source.id, index, width, source.revision) : null;
  return <ServerImage path={path} alt={`Слайд ${index + 1}: ${canvas.slide.title}`} className={`slide-thumb ${canvas.miniature ? 'miniature' : ''}`} fallback={<SlideCanvas {...canvas} />} />;
}
...
export default function App() {
  ...
  function navigate(next: Screen) {
    ...
    if (next === 'projects') {
      presentationService.recent(readOwn())
        .then(items => setDecks(items.map(item => item.deck)))
        .catch(e => setError(e instanceof Error ? e.message : 'Не удалось загрузить презентации.'));
    }
  }
  ...
  async function layoutDeck() {
    ...
    try {
      ...
      rememberOwn(projects.map(item => item.id));
  ...
  async function uploadTemplate(file?: File) {
    ...
    try {
      ...
      setTemplates(prev => [...prev.filter(existing => existing.id !== t.id), t]); updateBrief('templateId', t.id); setToast('Шаблон разобран: дизайн-система, макеты и картинки готовы к вёрстке'); setError('');
  ...
  return <div className={`app-shell simplified ${screen === 'create' ? 'home-shell' : ''} ${screen === 'editor' ? 'editor-shell' : ''}`}>
    <div className="app-main">
      <header className="simple-header">
        ...
        <div className="simple-header-right"><span className="demo-badge">{templates.length ? count(templates.length, 'шаблон', 'шаблона', 'шаблонов') : 'Подключаемся'}</span>{workflow && <div className="workflow-badge" ref={workflowCard}><button type="button" aria-expanded={workflowOpen} aria-haspopup="dialog" onClick={() => setWorkflowOpen(!workflowOpen)}>Воркфлоу {workflow.version}</button>{workflowOpen && <div className="workflow-card" role="dialog" aria-label={`Сценарий генерации ${workflow.version}`}><strong>Сценарий генерации {workflow.version}</strong><p>Текст, иллюстрации и проверки готовят разные агенты. Рядом — файл промпта и начало его sha256: по ним видно, какой в точности версией собрана колода.</p><ul>{workflow.agents.map(agent => <li key={agent.file}><span>{agent.role}</span><code>{agent.file}</code>{agent.sha && <em title="Начало sha256 промпта">{agent.sha}</em>}</li>)}</ul>{workflow.latest && <div className="workflow-change"><span>Что изменилось в {workflow.latest.version}</span><p>{workflow.latest.why}</p></div>}</div>}</div>}<IconButton label="Как это работает" onClick={() => setDialog('help')}><CircleHelp size={19} /></IconButton></div>
      </header>
      ...
      <main className={`main-content ${screen === 'editor' ? 'editor-main' : ''}`}>
        ...
        {screen === 'create' && <section className="creation-home">
          ...
          <div className="composer-meta"><span>{count(brief.slideCount, 'слайд', 'слайда', 'слайдов')} <i /> {currentTemplate.name}</span><span>Ctrl + Enter</span></div>
