  ...
  return <div className={`app-shell simplified ${screen === 'create' ? 'home-shell' : ''} ${screen === 'editor' ? 'editor-shell' : ''}`}>
    <div className="app-main">
      <header className="simple-header">
        ...
        <div className="simple-header-right"><span className="demo-badge">{templates.length ? `${templates.length} шаблона` : 'Подключаемся'}</span>{workflow && <div className="workflow-badge" ref={workflowCard}><button type="button" aria-expanded={workflowOpen} aria-haspopup="dialog" onClick={() => setWorkflowOpen(!workflowOpen)}>Воркфлоу {workflow.version}</button>{workflowOpen && <div className="workflow-card" role="dialog" aria-label={`Сценарий генерации ${workflow.version}`}><strong>Сценарий генерации {workflow.version}</strong><p>Текст, иллюстрации и проверки готовят разные агенты. Рядом — файл промпта и начало его sha256: по ним видно, какой в точности версией собрана колода.</p><ul>{workflow.agents.map(agent => <li key={agent.file}><span>{agent.role}</span><code>{agent.file}</code>{agent.sha && <em title="Начало sha256 промпта">{agent.sha}</em>}</li>)}</ul>{workflow.latest && <div className="workflow-change"><span>Что изменилось в {workflow.latest.version}</span><p>{workflow.latest.why}</p></div>}</div>}</div>}<IconButton label="Как это работает" onClick={() => setDialog('help')}><CircleHelp size={19} /></IconButton></div>
      </header>
