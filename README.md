...
flowchart LR
    ...
    C[Контент-пакет<br/>тексты · картинки] --> G
    C --> V[Ассеты<br/>иконки · иллюстрации]
    T --> V
    V --> L
    B[Бриф · назначение] --> G[Структура<br/>LLM со строгой схемой]
    P --> L[Вёрстка<br/>3 варианта на макетах шаблона]
    G --> L
    L --> A[Аудит<br/>38 кодов находок + проверка модели]
...
| **Вёрстка** | `backend/designer/layout.py` | три варианта `classic`, `split`, `focus` из одной структуры, с полями и кеглями самого шаблона; фактоиды, списки с иконками, текст с иллюстрацией |
| **Ассеты** | `backend/designer/assets.py` | иконки, иллюстрации и фото из шаблона, контент-пакета и встроенного SVG-набора; подбор по смыслу тезиса, SVG → нативные фигуры |
...
| **Интерфейс** | `frontend/src/App.tsx` | один экран на весь пайплайн; бренд-система и две темы — в `frontend/src/brand/` |
...
Модель не зашита в код. `DESIGNER_LLM_BASE_URL`, `DESIGNER_LLM_MODEL` и `DESIGNER_LLM_API_KEY` задают любого OpenAI-совместимого провайдера: vLLM, инференс VK или облачный API. Допустимы только открытые веса до 35B — например, `gpt-oss-20b` или `Qwen3-32B`. Подробности и ограничения — в [MODELS.md](MODELS.md). Для заранее настроенного профиля VK добавлен переключатель `DESIGNER_LLM_PROVIDER=vk`; порядок настройки, проверки и отката — в [инструкции деплоя](docs/deploy.md#переключение-на-vk). Живая совместимость VK и генерация за пять минут ещё требуют проверки с доступом организаторов.
...
backend/
  designer/
    ...
    assets.py       иконки и иллюстрации: сбор, подбор, отрисовка
    assets/pack/    встроенный набор: 63 иконки и 13 иллюстраций в SVG
    app.py          HTTP API
    storage.py      SQLite и файлы ревизий
    prompts/        промпты агентов + manifest.json
  tests/            сквозные тесты пайплайна
samples/            девять эталонных колод, материал и пример визуального пакета
tools/              сборка эталонов и прогон вёрстки по папке шаблонов
frontend/
  src/App.tsx       экраны: главная с брифом, структура, выбор подачи, редактор,
                    шаблоны, мои презентации
  src/services/     клиент API и адаптер, который связывает экраны с сервисом
...
| [docs/visual-assets.md](docs/visual-assets.md) | иконки, иллюстрации, фактоиды: источники, подбор, композиции |

## 🤝 Разработка

Ветки `feat/*` и `fix/*`, в `main` репозитория SourceCraft — через предложение изменений с зелёным CI (проверки идут на GitHub-зеркале, см. [docs/deploy.md](docs/deploy.md)). Промпты и конфиги моделей живут отдельными файлами, секреты — только в `.env`.
...
Код проекта — под [MIT](LICENSE). Сторонние библиотеки, шрифты и модели перечислены с
лицензиями в [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md); копилефтных (GPL, AGPL) среди
встраиваемых зависимостей нет. Правило ТЗ о лицензиях касается генеративных моделей — у нас
это открытые веса под Apache 2.0, подробности в [MODELS.md](MODELS.md).
